...
// Author: Julian Sam
// URL: https://github.com/Julian-Sam/Unrolled-Linked-List-in-C
...
node* makeNewNode(ull* list)
{
  ...
  first->data = (elem*)malloc(sizeof(elem)*list->slotsPerNode);
  first->next = NULL;
...
{
  ull* newNode = (ull*)malloc(sizeof(ull));
  if (newNode == NULL)
  {
  ...
  newNode->numNodes = 1;
  newNode->totalElems = 0;
  newNode->slotsPerNode = slotsPerNode;
  newNode->nextAvailSpot = 0;
  node* first = makeNewNode(newNode);
